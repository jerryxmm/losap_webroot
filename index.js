...
//	var local = new Server(window.location.host.split(':')[0], window.location.port, false);
	var local = new Server('本机', window.location.port, false);
	local.startMonitor();
	...
	if (serverLst != undefined)
	{
		for(var i = 0; i < serverLst.length; i++)
		{
			...
			var foreigner = new Server(server.ip, server.port, true);
...
function loadRemoveServiceList() {
	...
	for(var i = 0; i < svcLst.length; i++){
		var item = "<a href=\"#\"  onclick=\"showRemoveConfirmDlg('{0}')\" class=\"list-group-item\" data-toggle=\"modal\" data-target=\"#removeConfirm\"><span class=\"glyphicon glyphicon-minus-sign pull-right\" style=\"color:red\"></span>{1}</a>".format(svcLst.svc, svcLst.svc);
		items += item;
	}
...
function dealForeignLogin(data) {
	...
	g_intance.localServer.addForeignerToServer(obj.cipher,g_intance.foreignIp, g_intance.foreignPort );
}
